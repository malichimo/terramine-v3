...
/**
 * Maximum ad attempts allowed per day (3 ads × 2 attempts each)
 */
...
export class DatabaseServicePhase2 {
  ...
  async addResourcesToPool(
    ...
    try {
      ...
      // First ensure the parent resource map exists (setDoc merge creates it if missing)
      await setDoc(userRef, {
        [resourceKey]: { common: 0, uncommon: 0, rare: 0, epic: 0 },
      }, { merge: true });

      // Now safely increment the nested fields
    ...
    try {
      ...
      if (remaining < 2) {
        return {
          ...
          message: `Only ${remaining} ad attempt${remaining === 1 ? '' : 's'} left today (need 2 for +2 turns)`,
        };
      }
